...
| `TRUST_PROXY_HEADERS` | Trust `X-Forwarded-For` / `X-Real-IP` headers for client IPs (first valid IP) | `0` |
...
When `TRUST_PROXY_HEADERS` is enabled, the first **valid** IP from
`X-Forwarded-For` or `X-Real-IP` headers is used for rate limiting; invalid
or missing values fall back to the direct client address to avoid spoofing.
